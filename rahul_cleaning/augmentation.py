...
# Paths
images_path = r'temp_images/images'
annotations_path = r'temp_images/labels'
...
transformations = [
    ...
    A.Compose([A.Rotate(limit=180, border_mode=cv2.BORDER_CONSTANT, p=1.0)], bbox_params=bbox_params)
]
...
#     A.Compose([A.HorizontalFlip(p=1.0)], bbox_params=bbox_params),
#     A.Compose([A.VerticalFlip(p=1.0)], bbox_params=bbox_params),
#     A.Compose([A.Rotate(limit=180, p=1.0)], bbox_params=bbox_params)
# ]
...
#             augmented = transform(image=image, bboxes=bboxes, class_labels=class_labels)
#         except Exception as e:
#             print(f"Skipping {img_name} with transform {i} due to error: {e}")
#             continue
...
#         # Save augmented label (clipped to [0.0, 1.0])
#         aug_label_path = os.path.join(output_annotations_path, os.path.splitext(aug_img_name)[0] + '.txt')
#         with open(aug_label_path, 'w') as f:
#             for label, bbox in zip(aug_labels, aug_bboxes):
